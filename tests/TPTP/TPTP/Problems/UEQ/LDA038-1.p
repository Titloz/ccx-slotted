%------------------------------------------------------------------------------
% File     : LDA038-1 : TPTP v9.0.0. Released v4.1.0.
% Domain   : LD-Algebras
% Problem  : Identity 35 in the equational theory of group conjugation
% Version  : Especial.
% English  :
...
cnf(sos04,axiom,
    product(product(product(A,B),C),product(product(product(C,A),product(C,B)),product(A,product(A,D)))) = product(product(product(product(A,B),C),C),product(A,product(B,product(A,D)))) ).
