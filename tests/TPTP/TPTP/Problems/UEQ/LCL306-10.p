%------------------------------------------------------------------------------
% File     : LCL306-10 : TPTP v9.0.0. Released v7.3.0.
% Domain   : Puzzles
% Problem  : Principia Mathematica 4.74
% Version  : Especial.
% English  :
...
% Rating   : 0.95 v8.2.0, 1.00 v7.3.0
...
% Comments : Converted from LCL306-3 to UEQ using [CS18].
%------------------------------------------------------------------------------
...
cnf(and_defn,axiom,
    and(P,Q) = fnot(or(fnot(P),fnot(Q))) ).

cnf(equivalent_defn,axiom,
    equivalent(P,Q) = and(implies(P,Q),implies(Q,P)) ).

cnf(prove_this,negated_conjecture,
    theorem(implies(fnot(p),equivalent(q,or(p,q)))) != true ).
