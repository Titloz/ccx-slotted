%------------------------------------------------------------------------------
% File     : LDA027-1 : TPTP v9.0.0. Released v4.1.0.
% Domain   : LD-Algebras
% Problem  : Identity 17 in the equational theory of group conjugation
% Version  : Especial.
% English  :
...
cnf(sos04,axiom,
    product(product(product(A,B),product(product(A,C),C)),product(product(product(A,B),A),product(B,D))) = product(product(product(A,B),product(A,C)),product(product(product(A,B),product(C,A)),product(B,D))) ).
